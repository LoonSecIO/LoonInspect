...
**Release scope lives in GitHub milestones.** Read
[the release planning policy](docs/BRANCHING.md#11-release-planning-milestones-labels-and-tags).
`future` (formerly `v5`) means planned but unscheduled: do not build it until explicitly
selected, and preserve its documented implementation path. Milestones plan releases;
tags identify shipped commits. Keep `main` releasable for independent v1.x updates.
