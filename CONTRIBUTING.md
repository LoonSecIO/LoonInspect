...
- **Frontend** — `npx tsc -b --noEmit`, `npx eslint .`, `npm test` (vitest, node
  ...
  `react-hooks/set-state-in-effect` is an error and follows calls out of the effect body:
  an `async` loader's statements before its first `await` still run synchronously with
  the effect, so a `setLoading(true)` at the top of one is flagged at the call site. Load
  data as a promise chain the effect starts, with every `setState` in a `.then` /
  `.catch` / `.finally` callback (#15).
