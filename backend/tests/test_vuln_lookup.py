...
database anything (#533, #684). Pure: the router alone on a bare app, the permission granted,
and a session that fails the test if the route reaches for it — the shape is refused before
the tier and before any query, so nothing here needs Postgres. What the route answers for an
id it accepts is `test_vuln_answer_db.py`'s."""
...
def _ask(client: TestClient, vuln_id: str) -> httpx.Response:
    # Encoded whole, as the page's `encodeURIComponent` sends the id a hand-edited URL carries.
    return client.get(f"/api/vulnerabilities/{quote(vuln_id, safe='')}")


@pytest.mark.parametrize("finding_id", LOOSE_IDS)
def test_other_digits_or_a_newline_after_the_id_is_refused_like_any_other_shape(finding_id: str) -> None:
    ...
    sentence, with only the id in it changed."""
    client = _client()
    fourth, answer = _ask(client, FOURTH), _ask(client, finding_id)
