"""The daily exchange's sad path against a real Postgres (INSPECT-0082).

`post_exchange` and the due-ness arithmetic are covered without a database in
test_sharing.py. What needs one is the property those tests cannot see: that a
failed exchange still lands a share-log row, because the row is what consumes the
day. An upstream answering 200 with a non-JSON body used to raise past `db.add`
entirely — no row, so `exchange_due` said yes again on the next tick and the
once-a-day exchange became a crash loop. Gated on RUN_DB_TESTS like the other
