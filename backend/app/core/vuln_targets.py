...
from app.core.vuln_answer import READ_HERE, answering_epoch
...
async def load_title_updates(
    db: AsyncSession,
    rows: Sequence[AppCatalogEntry | InstalledApp],
    *,
    corpus: VulnCorpus,
    ...
    query = (
        ...
        .where(
            ...
            AppCatalogEntry.vuln_signature == answering_epoch(epoch),
            JamfPatchTitle.name != "",
        )
    ...
    for target, kind, version_hash, reference, name in (await db.execute(query)).all():
        ...
        # `epoch` as given (#685): the caller's one read, or, left out, a read per line as before.
        effect = update_line(by_build[key], corpus=corpus, target=target, epoch=epoch)
        if effect is None:
            continue
