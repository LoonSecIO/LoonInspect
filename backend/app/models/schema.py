...
class EventOutbox(Base):
    ...
    summary_collected_at: Mapped[datetime | None] = mapped_column(
    ...
    __table_args__ = (
        ...
        Index(
            ...
            "created_at",
            "id",
            postgresql_where=text("event_type = 'device.inventory' AND summary_collected_at IS NULL"),
        ),
    )
...
class SubjectDeparture(Base):
    ...
    # What the Mac tail has already SAID on the wire (#495, #179 4.5) — emission marks, not fleet state,
    # which stays `left_the_fleet`. `notice_day` is the last `departed` notice out (0 before the first,
    # never past seven), so the tail runs one per UTC day; `removed_notified_at` fires the terminal once.
...
class InventorySummaryState(Base):
    ...
    summary_status: Mapped[str] = mapped_column(String(24), default="baseline")
    short_summary: Mapped[str | None] = mapped_column(Text, nullable=True)


class InventorySummaryMetric(Base):
    """Minute counters replace per-snapshot receipts for no-op inventory (#594)."""

    __tablename__ = "inventory_summary_metrics"
    tenant_id: Mapped[uuid.UUID] = tenant_id_column(primary_key=True)
    provider: Mapped[str] = mapped_column(String(32), primary_key=True)
    bucket_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), primary_key=True)
    status: Mapped[str] = mapped_column(String(24), primary_key=True)
    reason: Mapped[str] = mapped_column(String(64), primary_key=True, default="none")
...
class InventorySummaryJob(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_summary_metrics_window", "tenant_id", "provider", "created_at"),
    )
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid7)
    tenant_id: Mapped[uuid.UUID] = tenant_id_column(index=True)
