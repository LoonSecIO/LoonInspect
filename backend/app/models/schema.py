...
class InstalledApp(Base):
    ...
    # None rather than minting a digest every nameless app would share), and the column
    # was added with no backfill, so a row written before that migration carries NULL
    # until its next ingest restamps it. Indexed like its siblings: the exchange
    # aggregate groups apps and the corpus joins on it.
