...
A hint for which card opens selected, and since #404 a gate on one card. The
Settings > AI page offers the "via Docker Desktop" card only on a full match whose
``alias_resolves`` is true — the AWS pod offered a Mac-only card with no Mac anywhere
near it, and the full match alone never proved the card's default could work — and
there it also opens selected. Every other outcome says what was seen, says in one
sentence why that card is withheld, and leaves the remaining choice to the operator.
Nothing here refuses anything: a call naming ``apple_fm`` is judged, gated and logged
like any other, wherever this runs.
"""
