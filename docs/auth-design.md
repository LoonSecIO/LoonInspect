...
Show-once is the *weakest* posture any credential in this build gets, not the strictest. §4.7 once claimed an exemption from it for the webhook secret, on the grounds that admins need to re-read that one; #203 ruled the other way, and #406 settled it at show-once too — but shown from the browser that generated it, never from a response, so the server still returns the webhook secret zero times. Nothing here is exempt in the other direction.
...
### 4.7 Webhook authenticity — implemented (#14), write-only by ruling (#203)

`/webhooks/jamf/{connection_id}` authenticates with a per-connection shared secret as of #14. It previously accepted any POST from anyone who could guess an integer.

> **Rewritten 2026-09-01 on the ruling in #203.** What stood here described a credential LoonInspect *generated*, kept *re-readable* behind a gated audited reveal, and rotated across *two live values* with a grace window. None of the three ever shipped, and the section had been asserting them since before #14 landed. The ruling was to keep the shipped posture rather than build up to the prose: a reveal endpoint is a new read path to a stored secret, and #200 and #202 had just spent two changes establishing that stored credentials have none. Rejected alternatives, in the order they cost: generate + reveal + `previous_secret` grace window (a migration, an endpoint and a UI); generate + reveal, single value; generate + show once, never re-readable. All three were rejected for the same reason — each one reintroduces a value-bearing response for a credential that currently has none. The section below is what the code does, not what it should do.

> **Amended 2026-09-12 on the ruling in #406.** Kyle ruled that the setup panel generates the secret and shows it once, and that losing it is answered by an easy **Rotate** rather than by a reveal. It generates in the *browser*: the value is made there and travels only browser → server, in the PATCH that saves it, so the #203 invariant is untouched — no response carries the value, and "generate + show once" above was rejected for exactly the value-bearing response this design does not have. Rotation stays single-value.

**Storage and presentation.** The value lives in `MdmConnection.webhook_secret_encrypted`, Fernet-encrypted at rest by the `EncryptedString` column type. Jamf Pro's Header Authentication takes a JSON object of header names to values, so what the admin pastes there is the object **Set up** on the connection's Webhook collection shows — `{"X-API-Key": "<secret>"}` — and Jamf presents it back as the `X-API-Key` header. `Authorization: Basic` (username ignored, password compared) and `Authorization: Bearer` are accepted as fallbacks for senders configured that way; all three land at the same comparison. The secret deliberately never travels in the URL path, which would put the entire authentication into proxy access logs and `Referer` headers. [`jamf-webhooks.md`](jamf-webhooks.md) is the operator's half.

**The browser generates the value; the server never returns it.** Set up's *Generate secret*, *Rotate*, and *Turn on* (when no secret is set yet) make 32 bytes with `crypto.getRandomValues` and send them in the PATCH that saves them (`frontend/src/features/mdm/webhookSetup.ts`). The strength of this credential is no longer whoever typed it by default. It still can be: *Use a secret of my own* accepts a typed value, and `webhook_secret` is still an unconstrained string on the way in, with no minimum length and no complexity check — a deployment where someone typed `jamf123` still has a webhook endpoint an attacker can guess into. The panel says a generated secret is stronger; nothing enforces it.

**The secret is write-only, and there is no path that returns it.** No API response carries it. `GET /api/mdm/connections` reports `has_webhook_secret: bool` and nothing more. The setup panel (`WebhookSetupPanel.tsx`) shows the header object once, built from the browser's own copy of the value it just sent, only after the save succeeds, and drops it when the panel closes; it is never put in storage or a URL. The connection form no longer shows or sends the field at all. This is the show-once rule §4.3 applies to API tokens, with one difference that keeps the posture whole: a token comes back in its creation response, whereas this value never leaves the server in any response — what is shown is what the browser made.

The operational cost is real and belongs here rather than in a footnote: **an admin who loses the value cannot recover it.** Their route back is **Rotate**, which makes a new secret and shows its header once; every callback between that and the new header reaching Jamf Pro is rejected. That gap is the accepted price of keeping the no-read-path posture whole. Whether it is the right price is a judgement about a 1.5-admin shop that will notice the outage as "webhooks stopped", so the failure needs to stay findable: each rejection logs at WARNING with the connection id and a `reason` naming which refusal it was and what to check ([`troubleshooting.md`](troubleshooting.md) §7), and setting the field is audited as `connection.updated` with `webhook_secret` in its `changed` list — the field name, never the value.

**Rotation is single-value: the new secret retires the old one immediately.** There is one column and no `previous_secret`, so there is no changeover window at all. The moment the write commits, callbacks carrying the old value get the same 401 as an attacker, and they keep getting it until someone updates the Jamf Pro side by hand. The right order is therefore: Rotate in LoonInspect, then update Jamf Pro — Rotate says so before the click — and treat the seconds in between as expected downtime for that connection. A grace window would need a migration, which this section once argued was cheaper before real deployments exist; that argument is now spent, and this build's answer is one column. `test_rotation_takes_effect_and_retires_the_old_value` pins it as intended behavior rather than an accident.

**This authenticates the caller, not the payload.** Unlike an HMAC signature over the body, a static header value gives no body integrity and no replay protection. That is an acceptable trade for Jamf compatibility — Jamf Pro does not sign its webhooks, so there is nothing to verify cryptographically — but it means TLS is load-bearing rather than defense-in-depth: the value is replayable verbatim by anyone who intercepts one callback.

**Comparison is constant-time and fails closed, and every rejection answers identically.** `hmac.compare_digest` via `tokens_equal`. A missing connection, an inactive one, one without the webhook capability, one with no secret set, and a plain wrong secret all return the same 401 — a request with no secret to compare against is compared against a module-level dummy instead, so the branch costs the same. Without that, response timing turns the small sequential integer in the path into a connection-enumeration oracle. Fail-closed on an unset secret is the deliberate half: an operator who enables the capability and has not yet set a value gets an endpoint that rejects everything, never one that accepts everything. The *log* says which rejection it was (#406): `rejection_reason` runs only after the comparison has already said no, from what the handler already holds, so nothing it decides reaches the caller or the response time. The body is read only after the secret matches, too, so an unauthenticated request's body is never parsed and a malformed one earns the same 401 as the rest; an authenticated body that is not JSON — a Jamf webhook left on XML — is a 422 that says so.
...
- `CONNECTION_CREDENTIAL_READ` covers **using a stored secret without being shown it**. Today that is exactly one route: `POST /connections/test`, which falls back to the stored credential when the payload omits one. The name is now wider than the thing — it was drafted expecting the webhook re-reveal §4.7 used to describe, and #203 ruled that reveal out, so nothing in the build reveals a secret value to anybody. The permission is kept under its original name rather than renamed: the line it draws — "may exercise a credential you may not read" — is the one that matters, and a rename would churn `permissions.py`, the frontend `PERMISSIONS` map, and every role fixture to say the same thing.
...
Those two pin the secret to a URL; #131 bounds which URLs may be stored in the first place, because "wherever `base_url` points" was previously any address the container could reach — cloud instance metadata included. `app/core/egress.py` holds the rule and the reasoning: https only (plain http is an explicit `ALLOW_INSECURE_MDM_BASE_URL` opt-in, since the client-credentials POST carries the secret in its body), no credentials or query in the URL, and loopback, link-local, unspecified, multicast and reserved addresses refused by literal and, best-effort, by what a hostname resolves to. Private ranges stay allowed on purpose — on-premises Jamf Pro lives there, and a rule that refuses `https://jamf.corp.internal` breaks a legitimate install to prevent an attack that install's own admin has no need to mount. `POST /connections/test` additionally bounds what comes *back*: a JSON error body up to 500 characters, and for anything else the status and content type only, so the endpoint stays diagnosable without being a read of whatever the caller pointed it at.
...
- **Webhook authentication** → per-connection static header credential (`X-API-Key`), pasted into Jamf Pro as the header object Set up shows. Was deferred; since shipped as #14. **Write-only, single-value rotation** — ruled on #203 after this line and §4.7 were found describing a generate-and-reveal design that never shipped — and **generated in the browser, shown once, with Rotate as the recovery** since #406. See §4.7.
