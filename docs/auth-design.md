...
Show-once is the *weakest* posture any credential in this build gets, not the strictest. §4.7 once claimed an exemption from it for the webhook secret, on the grounds that admins need to re-read that one; #203 ruled the other way, and the webhook secret is now shown zero times rather than once. Nothing here is exempt in the other direction.
...
**Storage and presentation.** The value lives in `MdmConnection.webhook_secret_encrypted`, Fernet-encrypted at rest by the `EncryptedString` column type. The admin sets the same value on the Jamf Pro side as the webhook's Header Authentication value — Jamf Pro supports a header name and a static value natively — and Jamf presents it back as `X-API-Key`. `Authorization: Basic` (username ignored, password compared) and `Authorization: Bearer` are accepted as fallbacks for senders configured that way; all three land at the same comparison. The secret deliberately never travels in the URL path, which would put the entire authentication into proxy access logs and `Referer` headers.

**The admin supplies the value; LoonInspect does not generate one.** There is no generator, no minimum length, and no complexity check — `webhook_secret` is an unconstrained string on the way in. The honest consequence is that the strength of this credential is whoever typed it, and the document should not imply otherwise: a deployment where someone typed `jamf123` has a webhook endpoint an attacker can guess into. Operators should paste 32 bytes of CSPRNG output (`openssl rand -base64 32`); nothing in the product enforces that today. Making the app offer a strong value is the cheapest of the rejected options above and remains available later — it is additive, and it would not create a read path, because a generated value shown once at creation is not a value read back.

**The secret is write-only, and there is no path that returns it.** No API response carries it. `GET /api/mdm/connections` reports `has_webhook_secret: bool` and nothing more; the form in `ConnectionForm.tsx` never seeds the field from the server, and a blank field on an edit means "keep what is stored" because the form omits the key rather than sending an empty string. This is stricter than the show-once rule §4.3 applies to API tokens — a token is at least displayed once at creation, whereas this value is never displayed at all, because it was never the app's to display.

The operational cost of that is real and belongs here rather than in a footnote: **an admin who loses the value cannot recover it.** Their only route back is to write a new secret and re-paste it into Jamf Pro, and every callback between those two moments is rejected. That gap is the accepted price of keeping the no-read-path posture whole. Whether it is the right price is a judgement about a 1.5-admin shop that will notice the outage as "webhooks stopped", so the failure needs to stay findable: rejections log at WARNING with the connection id, and setting the field is audited as `connection.updated` with `webhook_secret` in its `changed` list — the field name, never the value.

**Rotation is single-value: the new secret retires the old one immediately.** There is one column and no `previous_secret`, so there is no changeover window at all. The moment the write commits, callbacks carrying the old value get the same 401 as an attacker, and they keep getting it until someone updates the Jamf Pro side by hand. The right order is therefore: save in LoonInspect, then update Jamf Pro, and treat the seconds in between as expected downtime for that connection. A grace window would need a migration, which this section once argued was cheaper before real deployments exist; that argument is now spent, and this build's answer is one column. `test_rotation_takes_effect_and_retires_the_old_value` pins it as intended behavior rather than an accident.
...
- **Webhook authentication** → per-connection static header credential (`X-API-Key`-style), supplied by the admin and pasted into Jamf Pro. Was deferred; since shipped as #14. **Write-only, single-value rotation, no generator** — ruled on #203 after this line and §4.7 were found describing a generate-and-reveal design that never shipped. See §4.7.
