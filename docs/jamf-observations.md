...
**A Mac is its own category** (#183, from the same ruling). Computers are a subject kind like
the other two; the evidence and the consequence are stricter. The census closes a **device
sweep**, and only a *clean* one: it succeeded, it carried no RSQL `selector` — a scoped sweep
says nothing about the Macs it never asked for — and no device failed, because a device Jamf
*did* return but whose ingest failed has a stale `last_seen_at`, so a dirty night judges
nobody. It names every computer the sweep returned across every page, **including the reads
skipped as stale**: the monotonic guard is about what a record says, not whether the Mac is
there, so a stale-skipped read stamps `last_seen_at` and counts as presence (rider 3). A
sweep that was not a census says which of the three disqualified it; a census logs *device
census: N observed, D departed, R returned, T in their seven-day tail, L left the fleet*.

The consequence is the **seven-day tail**: an open row *is* the tail, and seven days after
`departed_at` the Mac has **left the fleet** — out of `GET /api/devices` (`includeDeparted=true`
reads one back) and out of the device count on `/api/mdm/status` and the Overview. No column and
no timer: a row's age is the whole state, counted in one place (`left_the_fleet` in
`app.observations.departure`), so two surfaces cannot disagree about one Mac. **Held**: the device
row, its spans, its sections and its whole change history — erasure is ruled in principle and
deferred to #180 (v5), and this stamp is what that pass selects on. **Not moved**: the `devices.*`
posture keys, which still count every row; redefining that population is #135's open ruling. The
daily "device is gone" emission and any "device returned" event are #179's shape, and are not built.
...
Jamf Patch answer per app), `docs/app-catalog.md` (the tenant's distinct apps, first/last seen,
where that answer lives), `docs/splunk-event-shaping.md`
