export const en = {
  ...
  vulnerabilities: {
    ...
    // #591, off the finding ledger. Findings and not Macs: one Mac carrying one id through two
    // apps is two findings, the grain the ledger keeps.
    ...
    // #591 — the ledger's two clocks: FIRST is when this pod first saw the id on a Mac (bounded by
    // this organization's own history, never the CVE's publication date), LAST is as of each Mac's
    // last observation, since an open finding is written once. `detectedNone` is the absence.
    detected: (first: string, last: string) => `First detected here ${first} · last detected ${last}.`,
    detectedMacs: (open: number, ever: number) =>
      `${open} Mac${open === 1 ? "" : "s"} still carry it of ${ever} that ever did — as of each Mac's last observation, not as of now.`,
