...
export const en = {
  ...
  system: {
    ...
    sharing: {
      ...
      moreCaseMisses: (count: number) => `…and ${count} more, also not matched (case)`,
      ...
      // Empty is a state with more than one cause, and they are not one sentence (#483).
      // With no catalog synced only the library can make a title known, so an empty list
      // there is about missing inventory and not about a fleet nothing knows.
      candidatesNone: (catalog: number, library: number) =>
        catalog === 0
          ? `Nothing to suggest, and little to suggest it from: this container has synced 0 Jamf Patch titles, so only the ${library} vulnerability-library titles it holds can make a title known here. An empty list under those counts means no application inventory has been collected yet, or that the apps collected carry no bundle identifier — sweep a connection, then troubleshooting.md §6 step 4.`
          : `No candidates: every title this fleet carries with a bundle identifier is known to a public source here, checked against ${catalog} Jamf Patch titles and ${library} vulnerability-library titles.`,
      candidatesLoading: "Counting what each pattern matches…",
      // Said, not swallowed: a panel that vanished reads as a build without one (#150).
      candidatesFailed:
        "Match counts and candidates could not be loaded, so nothing is being said here about this fleet — the box above still saves. Check docker compose logs app, then reload the page.",
      ...
      groupSummary: (apps: number, devices: number) =>
        `${apps} unknown title${apps === 1 ? "" : "s"} on ${devices} Mac${devices === 1 ? "" : "s"}`,
      groupCovered: "already covered by a pattern in the box",
      addGlob: (glob: string) => `Add ${glob}`,
      moreGroups: (count: number) =>
        count === 1 ? "1 more prefix is not shown." : `${count} more prefixes are not shown.`,
  ...
  destinations: {
    ...
    httpInfoLabel: "Why HTTP is discouraged",
    httpInfo: "HTTP sends credentials and events without encryption. Prefer HTTPS. Hosts ending in .internal, localhost, 127.0.0.1, and IPv6 loopback ::1 are permitted by the host policy, but HTTP still needs a transport opt-in. In Docker, loopback points to the app container; use host.docker.internal to reach the Docker host.",
    transport: "Transport security",
    requireHttps: "Require HTTPS",
    allowHttp: "Allow HTTP (lab only)",
    inheritTransport: "Use deployment default (legacy)",
    httpWarning: "HTTP sends credentials and events without encryption. Allow only for a trusted lab destination without TLS.",
    inheritHint: "Uses ALLOW_INSECURE_DESTINATION_URL on each delivering container. Choose an explicit setting for consistent behavior across containers.",
    httpLabel: "HTTP · unencrypted",
    url: "URL",
    urlHintSplunk:
  ...
  ai: {
    inventorySummary: {
      ...
      "preference": "Customer emphasis / tone",
      "placeholder": "Emphasize encryption regressions and new critical findings.",
      "preferenceHelp": "/500 characters. Preference affects prose only; it cannot authorize actions or change deterministic evidence.",
      "pause": "Apple FM pause between background calls (seconds)",
      "save": "Save inventory settings",
      "saving": "Saving…",
      "saved": "Inventory summary settings saved.",
      "loadError": "Could not load inventory summary settings.",
      "saveError": "Could not save settings.",
      "metricsTitle": "Inventory AI",
      "providerUnavailable": "Provider unavailable",
      "unavailable": "Summary metrics unavailable. Last values may be stale.",
      "window": "Last 24 hours · one-minute counters · operational measurements, not a capacity benchmark",
      "success": "Successful jobs / attempts",
      "dropped": "Dropped or failed / received",
      "overloads": "Rate-limit responses",
      "latency": "Mean latest-attempt latency",
      "queued": "Queued",
      "cached": "Cache hits",
      "noUpdates": "No updates",
      "attempts": "Attempts",
      "oldest": "Oldest queued",
      "none": "None",
      "asOf": "As of",
      "diagnostics": "Outcomes requiring attention"
    },
    title: "AI",
    ...
    detectionDockerDesktopMac: "Docker Desktop on macOS detected.",
    ...
    // #404: the card list follows the reading now, so the panel says when a card is held
    // back and why — a withheld card must not read as a feature this build lost.
    ...
    providerHelp: {
      apple_fm:
        "Click here if LoonInspect runs under Docker Desktop on this Mac and Apple's own `fm serve` is running (macOS 27; port 1976).",
      openai_compatible:
    ...
    // The same card where there is no Mac host to default to (#404).
    openaiHelpNoLocalDefault:
      "OpenAI itself, a gateway, LM Studio, vLLM, or an Ollama this container can reach. Bring your own URL and key.",
    appleGuide: "How to set up Apple's on-device model with Docker Desktop",
