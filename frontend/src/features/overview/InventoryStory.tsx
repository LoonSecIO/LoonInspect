...
interface Cells {
  ...
  catalog: Cell<CatalogSummary | null> | null;
  topApps: Cell<Application[]> | null;
}

/**
 * The inventory-read story on `/` (#115, re-scoped by Kyle 2026-09-16).
 *
 * There is no `/dashboards` and R6 ruled one out of v1, so a viewer's first ten minutes
 * live here: the front page of an account without `destination:read` is this board rather
 * than the sentence it used to be. Which tiles it holds is `overviewPlan.planOverview`'s
 * decision — each planned against the permission its own source demands, so no tile is
 * rendered into a 403.
 *
 ...
 * a viewer session left running on a wall display. A source that stops answering says so
 * in its own tile — failure must never read as emptiness (#150).
 */
export function InventoryStory({ tiles }: { tiles: readonly InventoryTile[] }) {
  ...
  const load = useCallback(() => {
    ...
    void Promise.all([
      ...
      ask("catalog", () => listCatalog({ pageSize: 1 }).then((page) => page.summary)),
  ...
  const card = (tile: InventoryTile): ReactNode => {
    switch (tile) {
      case "fleet":
        return (
          <Tile key={tile} title={ti.fleetTitle} href="/devices" asOf={readAt}>
            {resolve(cells?.fleet, words, (n) => (n === 0 ? ti.fleetNone : ti.fleetCount(n)))}
          </Tile>
        );
      // The hygiene counts (#109), reused rather than restated: the same saved searches an
      // operator sees, so a viewer and an analyst never read two different numbers.
      case "hygiene":
        return (
        ...
        return (
          <Tile key={tile} title={ti.catalogTitle} href="/devices/applications/catalog" asOf={readAt}>
            {resolve(cells?.catalog, words, (s) =>
              s === null || s.entries === 0 ? ti.catalogNone : ti.catalogCounts(s.entries, s.matched, s.unmatched)
            )}
          </Tile>
        );
      case "topApps":
...
/** A claim, a body, the stamp that ages it, and the link to the page the count came from
 *  — every tile here IS a saved search, the hygiene tiles' rule. */
