...
/** #472's object, as much of it as the page reads; every figure printed is a field here. Contract and
 *  vocabulary: docs/compliance-evidence.md. `days` is absent where it rounds to zero, and `fleet` is
 *  `{}` where nothing was counted — absent, never zero (§4). */
 ...
 *  `seconds` it is asserted on, never on `days`, which round to two places. `null` when it closes and for
 *  a bucket that counted nothing (a `{}` is absent, not a failed sum); both figures otherwise, so the
 *  page can print the fault rather than hide it. */
...
/** The download, moved here from the Settings › Connections row (#536) and otherwise the same fetch: raw
 *  rather than `apiRequest` because the endpoint answers HTML, and the server names the file. A refusal
