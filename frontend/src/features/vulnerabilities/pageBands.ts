...
/** The finding ledger's three (#591), their own tuple for the reason the recorder keeps them in one: they are absent
 *  for a DIFFERENT reason than the four — those while nothing has judged this organization, these while the ledger holds
 *  no row. The band's dash means either. */
...
/** The latest capture's rows, read as rows (#470). A key with no row is *absent*, a statement about that night and
 *  not the number nought: nothing here defaults, sums or fills a gap. The four are written only once a corpus has
 *  judged the organization and the three only once its finding ledger holds a row, so an empty read — and a partial
 *  one — is an ordinary first. */
